...
def main():
    with sync_playwright() as p:
        ...
        print("ICE", host.evaluate("() => window.BoberNet.refreshIce()"), guest.evaluate("() => window.BoberNet.refreshIce()"))
        print("LINKTEST_OK")
        browser.close()
        ...
        host = browser.new_page()
        guest = browser.new_page()
        host.goto(relay, wait_until="networkidle", timeout=30000)
        guest.goto(relay, wait_until="networkidle", timeout=30000)
        host.evaluate("() => localStorage.setItem('bober-yeet-war-tut', '1')")
        guest.evaluate("() => localStorage.setItem('bober-yeet-war-tut', '1')")
        host.click("#mode-link")
        host.click("#btn-host")
        ...
        host_ice = host.evaluate("() => window.BoberNet.refreshIce()")
        guest_ice = guest.evaluate("() => window.BoberNet.refreshIce()")
        print("RELAY ICE", host_ice, guest_ice)
        pair = (host_ice or {}).get("pair") or ""
        gpair = (guest_ice or {}).get("pair") or ""
        assert "relay" in pair or "relay" in gpair, (host_ice, guest_ice)
        host.click("#btn-play")
        host.wait_for_function("() => window.__yeetWar && window.__yeetWar.snapshot().phase === 'aim'", timeout=20000)
        guest.wait_for_function("() => window.__yeetWar && window.__yeetWar.snapshot().phase === 'aim'", timeout=20000)
        host.evaluate("() => { const w = window.__yeetWar; w.setWeapon('stick'); w.setAim(18, 36); w.fire(); }")
        guest.wait_for_function(
            "() => { const s = window.__yeetWar.snapshot(); return s.turn === 'creek' && s.phase === 'aim'; }",
            timeout=25000,
        )
        guest.evaluate("() => { const w = window.__yeetWar; w.setWeapon('stick'); w.setAim(160, 36); w.fire(); }")
        host.wait_for_function(
            "() => { const s = window.__yeetWar.snapshot(); return (s.turn === 'lodge' && s.phase === 'aim') || s.phase === 'end'; }",
            timeout=25000,
        )
        print("RELAY AFTER", snap(host)["phase"], snap(guest)["phase"])
        print("RELAY_OK")
        browser.close()
